...
describe('Functional check', function () {
            ...
            var domainPolling = function () {
              ...
              DomainHelpers.checkStatus(firstDc.id).then(function (res) {
                if (res.staging_status === 'Published' && res.global_status === 'Published') {
                  ...
                  firstDcFull.domain_wildcard_alias = '*.ls-test-alias.com';
                  return API.resources.domainConfigs
  ...
  describe('Destination S3, Source type Domain, Domain wildcard alias test', function () {
    ...
    it('should send requests to recently created domain config (domain wildcard alias) to generate logs in 2 minutes', function (done) {
      setTimeout(function () {
        ...
        productionProxyServers.forEach(function (server) {
          proxyRequests.push(
            utils.sendProxyServerRequest(server, '1' + firstDcFull.domain_wildcard_alias.substring(1), 80)
          );
          proxyRequests.push(
            utils.sendProxyServerRequest(server, '2' + firstDcFull.domain_wildcard_alias.substring(1), 80)
          );
          proxyRequests.push(
            utils.sendProxyServerRequest(server, '3' + firstDcFull.domain_wildcard_alias.substring(1), 80)
          );
        });
