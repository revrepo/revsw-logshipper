...
var deleteShippedFiles = config.delete_shipped_files;
//  ---------------------------------
...
JobsQueue.prototype.cleanBeforeStart = function () {
  ...
  return fs.readdirAsync(stuff.toUploadsPath(''))
    .then(function (files) {
      ...
      return (filesNum > 0 && deleteShippedFiles === true)? Promise.all(files.map(function (file) {
...
JobsQueue.prototype.loadNewJobs = function () {
  ...
  return LogShippingJobs.listShippingJobsAsync()
    ...
    .then(function () {
      ...
      pausedJobs = newjobs.filter(function (job) {
        return job.operational_mode === 'pause';
      });
    ...
    .then(function () {
      ...
      if (pausedJobs && pausedJobs.length && pausedJobs.length > 0) {
        // Notify master with stats
        process.send({
          type: 'stats',
          state: 'jobs_paused_count',
          jobs_count: pausedJobs.length
        });
      }
  ...
  return DomainLogs.listByJobsAsync(jobs)
    .then(function (logs) {
      ...
      jobs.forEach(function (job) {
        ...
        if (job.domain_wildcard_alias || (job.domain_aliases && job.domain_aliases.length)) {
        ...
        logger.info('JobsQueue.loadDomainLogs, job id ' + job.job_id + ', domain name ' + job.domain_name + ', loaded logs[' + job.logs.length + '] at ' + new Date());
    ...
    .then(function (jobsToFile) {
      ...
      var filedJobs = jobsToFile.filter(function (job) {
        if (job.status === stuff.SS_FILED) {
          ...
          logger.info('JobsQueue.saveLogsForUpload, job id ' + job.job_id + ', domain name ' + job.domain_name + ', filled');
          if (typeof job.shared.file_for_upload === 'string') {
            logger.info('JobsQueue.saveLogsForUpload, filename ' + job.shared.file_for_upload);
          }
          // Notify master with stats
          process.send({
            type: 'stats',
          ...
          logger.info('JobsQueue.saveLogsForUpload, job id ' + job.job_id + ', domain name ' + job.domain_name + ', not filled');
          // Notify master with stats
          process.send({
            type: 'stats',
            state: 'job_not_filed'
    ...
    .then(function (jobsToDispatch) {
      var shippedJobs = jobsToDispatch.filter(function (job) {
        if (job.status === stuff.SS_SHIPPED) {
          ...
          logger.info('JobsQueue.dispatchLogs, job id ' + job.job_id + ', domain name ' + job.domain_name +
            ', shipped');
          // Notify master with stats
          process.send({
            type: 'stats',
            state: 'job_shipped',
            job_type: job.destination_type
          });
          return true;
        } else {
          ...
          logger.info('JobsQueue.dispatchLogs, job id ' + job.job_id + ', domain name ' + job.domain_name +
            ', not shipped');
          logger.info('JobsQueue.dispatchLogs, job destination/host ' + job.destination_type + ' / ' +
            job.destination_host + ', credentials ' + job.destination_username);
          // Notify master with stats
          process.send({
            type: 'stats',
            state: 'job_not_shipped',
            job_type: job.destination_type
          });
        }
        return false;
...
JobsQueue.prototype.processShippedJobs = function () {
  ...
  if (!shippedJobs.length || deleteShippedFiles === false) {
    return Promise.resolve(false);
  }
...
JobsQueue.prototype.processFailedJobs = function () {
  ...
  return LogShipperJobs.saveJobsAsync(failedJobs)
    ...
    .then(function () {
      // NOTE: Attantion - if files was not deleted they will be overwritten 
      if(deleteShippedFiles === false){
...
JobsQueue.prototype.cleanLogs = function () {
  ...
  return LogShipperJobs.listByStatusAsync(stuff.SS_SHIPPED)
    .then(function (result) {
      logger.info('JobsQueue.cleanLogs ' + result.length + ' SHIPPED LogShipperJobs to clear');
      
      return DomainLogs.cleanByJobsAsync(result || []);
    })
    .then(function (result) {
      logger.info('JobsQueue.cleanLogs ' + result.n + ' DomainLogs deleted');
      return LogShipperJobs.cleanAsync();
    })
    .then(function (result) {
      jobsDeleted += result.n;
      logger.info('JobsQueue.cleanLogs ' + jobsDeleted + ' LogShipperJobs deleted');
    });
};
