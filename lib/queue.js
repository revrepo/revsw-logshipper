    ...
    .then(function (jobsToDispatch) {
      var shippedJobs = jobsToDispatch.filter(function (job) {
        if (job.status === stuff.SS_SHIPPED) {
          ...
          logger.info('JobsQueue.dispatchLogs, job id ' + job.job_id + ', domain name ' + job.domain_name + ', shipped');
          // Notify master with stats
          process.send({
            type: 'stats',
            state: 'job_shipped',
          ...
          logger.info('JobsQueue.dispatchLogs, job id ' + job.job_id + ', domain name ' + job.domain_name + ', not shipped');
          logger.info('JobsQueue.dispatchLogs, job destination/host ' + job.destination_type + ' / ' + job.destination_host + ', credentials ' + job.destination_username);
          // Notify master with stats
          process.send({
            type: 'stats',
...
JobsQueue.prototype.cleanLogs = function () {
  ...
  return LogShipperJobs.listByStatusAsync({$gte: stuff.SS_SHIPPED})
    .then(function (result) {
      jobsToClear = (result || []).map(function(job) {
        return job.job_id;
      });
    ...
    .then(function (result) {
      ...
      return LogShipperJobs.cleanByIdsAsync(jobsToClear);
    })
    .then(function (result) {
