...
var saveForUpload_ = function(job) {
  ...
  return Promise.resolve(job)
    // TODO: old version - delete after test new version
    // .then(function (job) {
    ...
    //   var escapedDomainName = job.domain_name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
    //   var zipper = new zip(),
    //     filename = job._id + '.' + escapedDomainName + '.log';
    //   zipper.addFile(filename, new Buffer(data),/*comment*/ '');
    //   job.shared.file_for_upload = filename+'.zip';
    //   // NOTE: Promise.promisifyAll not work zipper
    //   return  new Promise(function(resolve,reject){
    //     // for write everything to disk
    //     zipper.writeZip(/*target file name*/stuff.toUploadsPath(filename+'.zip'), function(err,data){
    //       if(err){
    //         return reject(err);
    //       }
    //       return resolve(data);
    ...
    .then(function(job) {
      return new Promise(function(resolve, reject) {
        ...
        var inStream = new streamReadable();
        var escapedDomainName = job.domain_name.replace(/[^a-z0-9]/gi, '_').toLowerCase();
        var fileName = job._id + '.' + escapedDomainName + '.log';
        var fileStream = fs.createWriteStream( /*target file name*/ stuff.toUploadsPath(fileName + '.zip'));

        _.forEach(job.logs, function(itemLog) {
          //  convert every record to json string + lf
          inStream.push(JSON.stringify(itemLog) + '\n');
        });
...
var S3Fire_ = function(job, retries) {
  ...
  return new Promise(function(resolve, reject) {
      s3.upload({
          Key: job.shared.file_for_upload,
          Body: body
        })
