...
var LogShipperDB = module.exports = {
  ...
  health: function() {
    ...
    return ( db_ ? promise.resolve( true ) : this.connect() )
      .then( function() {
        return fs.readFileAsync( stuff.toRootPath( config.version_file ), { encoding: 'utf8' });
      } )
      .then( function( v_ ) {
        version = v_.trim();
        return promise.all( [
      ...
      .then( function( c_ ) {
        return { good: true, version: version };
      } )
      .catch( function( err ) {
        logger.error( 'LogShipperDB.health: ' + err.toString() );
        // gulp exception and just return false
        return { good: false, version: version, msg: err.toString() };
      } );
  },
  ...
  collectAllDomainLogs: function( jobs ) {
    ...
    var span = stuff.makeSpan(),
      coll,
      where = {
        domain: {},
        unixtime: { $gte: span.from, $lte: span.to },
    ...
    return ( db_ ? promise.resolve( true ) : this.connect() )
      .then( function() {
        ...
        return coll.find( where ).project( { _id: 0 } ).sort( { unixtime: 1 } ).toArray();
      } )
      .then( function( logs ) {
   ...
   *  @param {string} new status name for logging
   *  @returns {Promise()}
   */
  updateDomainLogs: function( jobs, new_status ) {
    ...
    return ( db_ ? promise.resolve( true ) : this.connect() )
      .then( function() {
        ...
        where.$or = jobs.map( function( job ) {
          return {
            ...
            unixtime: { $gte: job.span.from, $lte: job.span.to }
          };
        } );
        logger.debug( 'where', where );
      ...
      .then( function( data ) {
        ...
        logger.info( 'LogShipperDB.updateDomainLogs: ' + data.result.nModified + ' records shipping status updated to ' + stuff.statusNames[new_status] );
        return jobs;
      } )
      .catch( function( err ) {
