...
var LogShipperDB = module.exports = {
  ...
  cleanLogs: function() {
    ...
    var coll,
      where = { $or: [
        { shipping: stuff.SS_SHIPPED },
        { unixtime: { $lte: ( Date.now() / 1000 - config.logs_max_age_hr * 3600/*sec*/ ) } }
      ] };

    // logger.debug( 'LogShipperDB.cleanLogs, where', where );

    return ( db_ ? promise.resolve( true ) : this.connect() )
      .then( function() {
        coll = db_.collection( config.logshipper_db.domain_log_collection );
        return coll.deleteManyAsync( { unixtime: { $lte: ( Date.now() / 1000 - config.logs_max_age_hr * 3600/*sec*/ ) } }, { w: 1 } );
      } )
      .then( function( data ) {
        // logger.debug( 'result', data.result );
        //  { result: { ok: 1, n: 2100 },
        logger.info( 'LogShipperDB.cleanLogs: ' + data.result.n + ' expired records deleted.' );
        return coll.deleteManyAsync( { shipping: stuff.SS_SHIPPED }, { w: 1 } );
      } )
      .then( function( data ) {
        logger.info( 'LogShipperDB.cleanLogs: ' + data.result.n + ' shipped records deleted.' );
        return true;
      } )
      .catch( function( err ) {
