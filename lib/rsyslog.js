      ...
      .then(function (stencil) {
        ...
        if (domains.length) {
          ...
          domains.forEach(function(domain) {
            if (domain.indexOf('*') === -1) {
              domains += '$!domain == \'' + domain + '\' or\n';
            } else {
              domains += '$!domain contains \'' + domain.substring(1) + '\' or\n';
            }
          });
          // remove last or \n
          domains = domains.substring(0, domains.length - 3);
          
          stencil = stencil.replace(config.rsyslog.domains_list_tag, domains);
          if (tagpos !== -1) {
