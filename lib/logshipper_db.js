...
var LogShipperDB = module.exports = {
   ...
   *  @param {integer} linux epoch time in seconds, optional
   *  @param {integer} linux epoch time in seconds, optional
  ...
  collectAllDomainLogs: function( jobs ) {
    ...
    return ( db_ ? promise.resolve( true ) : this.connect() )
      .then( function() {
        ...
        where.domain = { $in: jobs.map( function( job ) {
      ...
      .then( function( logs ) {
        logger.info( 'LogShipperDB.collectLogs: ' + logs.length + ' log records downloaded.' );
        if ( logs.length ) {
          ...
          return coll.updateAsync( where, { $set: { shipping: stuff.SS_COLLECTED } }, { w: 1, multi: true })
            .then( function( data ) {
              ...
              logger.info( 'LogShipperDB.collectLogs: ' + data.result.nModified + ' records shipping status updated.' );
              return logs;
            })
        }
        return logs;
